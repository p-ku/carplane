...
const float intensity = 10.;							// how bright the light is, affects the brightness of the atmosphere
...
uniform sampler2D color_buffer : hint_albedo; // Velocity and depth information
...
void fragment()
{
	...
	vec4 atm = calculate_scattering(cam_pos[3].xyz, normalize(mat3(cam_pos) * view.xyz), length(view.xyz), light_direction, depth);
	atm.w = clamp(atm.w, 0.000000001, 1.);

	// COLOR = vec4(atm.xyz / atm.w, atm.w); // + vec4(base_color.xyz * atm.w, 1. - atm.w);
	// COLOR = vec4((atm.xyz + 1. - base_color.xyz), 1.);
	// COLOR = vec4(base_color.xyz * (1. - atm.w) + atm.xyz * 10., 1.);
	vec4 base_color = texture(color_buffer, SCREEN_UV);

	//	COLOR = vec4(base_color.xyz * (1. - atm.w) + atm.xyz, 1.);
	//	COLOR = vec4(atm.xyz / atm.w, atm.w);
	//	vec4 blend = vec4(atm.xyz / atm.w, atm.w);
	//	// blend = clamp(blend, 0., 1.) * atm.w;
	//	COLOR = vec4(base_color.xyz * (1. - blend.w) + blend.xyz, 1.);
	// COLOR = mix(2. * base_color * blend, 1. - 2. * (1. - base_color) * (1. - blend), step(0.5, blend));
	//	COLOR = 1. - (1. - base_color) * (1. - blend);
	vec4 blend = vec4(atm.xyz / atm.w, atm.w);

	ALBEDO = vec3(base_color.xyz * (1. - atm.w) + atm.xyz);
	ALBEDO = vec3(atm.xyz / atm.w);
	// blend = clamp(blend, 0., 1.) * atm.w;
	ALBEDO = vec3(base_color.xyz * (1. - blend.w) + blend.xyz);
}